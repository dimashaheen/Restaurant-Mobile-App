...
const FriedRolls = () => {
 ...
 const [nameInTheCart , setNamesInTheCart] = useState([]);
  ...
  useEffect(() => {
    Promise.all([
      axios.default.get(`http://192.168.8.105:3000/items/Fried Rolls`),
    ])
...
const store = async(nameInTheCart , prciesInTheCart) => {
    try {
      ...
      await AsyncStorage.setItem("FriedRollsNames", nameInTheCart)
      await AsyncStorage.setItem("FriendRollsPrices", pricescartItems)
